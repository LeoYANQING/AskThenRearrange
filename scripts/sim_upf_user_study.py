"""Simulate 10 UPF trials and dump the full QA transcript per episode.

Uses the same pipeline Study 2 ships with: policy controller in UPF mode,
PreferenceElicitingProposer / ActionProposer / PreferenceInductionProposer from
proposers.py, NaturalUserOracle for simulated answers, and the shared
state_update. Budget is fixed at 6 (Study 2 default).

Output:
  - per-episode banner + every turn's pattern, question, simulated answer
  - end-of-run summary: pattern distribution across all turns
"""
...
# Controls which room slice this run uses. Pass --offset N to start at index N
# per room (default 0 = first three of each room).
...
def run_episode(episode, controller, ao, pe, pi, oracle, updater):
    state = build_initial_state(episode=episode, strategy="user_preference_first", budget_total=BUDGET)
    turns = []
    while len(state["qa_history"]) < BUDGET:
        decision = controller.plan_next_question(state=state, mode="user_preference_first")
        if decision is None:
            break
        intent = propose(state, decision, ao, pe, pi)
        if intent is None:
            # Mirror dialogue.py fallback: any proposer failure → AO.
...
def main():
    ...
    out_tag = os.environ.get("SIM_TAG", "run1")
    episodes = pick_episodes(offset)
    print(f"Running {len(episodes)} UPF episodes, budget={BUDGET}, offset={offset}, tag={out_tag}")
    use_study2 = os.environ.get("SIM_PE_VARIANT", "study2") == "study2"
    controller = QuestionPolicyController(selection_method="rule")
    ao = ActionProposer()
    pe = Study2PreferenceElicitingProposer() if use_study2 else PreferenceElicitingProposer()
    print(f"PE proposer: {type(pe).__name__}")
    ...
    for idx, ep in enumerate(episodes, 1):
        ...
        print(f"receptacles: {ep.receptacles}")
        print(f"seen_objects: {ep.seen_objects}")
        print(f"unseen_objects: {ep.unseen_objects}")
        ...
        for t in turns:
            ...
            print(f"  A: {t['answer']}")
            if t["oracle_receptacle"]:
                print(f"  oracle_receptacle: {t['oracle_receptacle']}")
        # Evaluate with canonical Study 1 placements (oracle answers were
        # grounded on those — matching ground truth is the fair yardstick).
        try:
        ...
        seen_scores.append(seen_acc)
        unseen_scores.append(unseen_acc)
        total_scores.append(total_acc)
        ...
        all_results.append({
            ...
            "turns": turns,
            "seen_psr": seen_acc,
            "unseen_psr": unseen_acc,
            "total_psr": total_acc,
        })

    print(f"\n{'=' * 80}")
    print("Pattern distribution across all turns:")
    ...
    out_path = ROOT / "logs" / f"sim_upf_user_study_{out_tag}.jsonl"
