...
try:
    ...
    from v2.proposers import ActionProposer
...
def run_action_loop(
    *,
    ...
    while state["budget_used"] < state["budget_total"]:
        ...
        if intent is None:
            print(f"[step {step_idx}] no action intent available")
            break
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Smoke test for ActionProposer + Oracle + StateUpdate.")
    ...
    if args.plot_curve:
        ...
        for budget in budgets:
            _, evaluation = run_action_episode(
                episode=episode,
                budget=budget,
        ...
        output_path = args.curve_output or "v2/plots/action_loop_accuracy_curve.png"
        saved_path = plot_accuracy_curve(
            curve_points,
            output_path=output_path,
            title=f"Action Loop Accuracy vs Budget ({episode.episode_id})",
        )
        print(json.dumps({"curve_points": curve_points, "saved_plot": saved_path}, indent=2, ensure_ascii=False))
        return

    for budget in budgets:
        final_state, evaluation = run_action_episode(
            episode=episode,
            budget=budget,
            proposer_model=args.proposer_model,
            oracle_model=args.oracle_model,
